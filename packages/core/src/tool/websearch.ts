...
export class ConfigService extends Context.Service<ConfigService, Config>()("@klautcode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.KLAUTCODE_WEBSEARCH_PROVIDER === "exa" || process.env.KLAUTCODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.KLAUTCODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("KLAUTCODE_EXPERIMENTAL") || truthy("KLAUTCODE_ENABLE_EXA") || truthy("KLAUTCODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("KLAUTCODE_ENABLE_PARALLEL") || truthy("KLAUTCODE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `klautcode/${InstallationVersion}`,
