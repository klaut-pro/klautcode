...
export class Service extends Context.Service<Service, Interface>()("@klautcode/v2/Snapshot") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const worktree = source
      ? AbsolutePath.make(
          yield* fs.realPath(source.worktree).pipe(
            Effect.catchReason("PlatformError", "NotFound", () => Effect.succeed(source.worktree)),
            Effect.orDie,
          ),
        )
