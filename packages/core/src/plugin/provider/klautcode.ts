...
import type { IntegrationOAuthMethodRegistration } from "@klautcode/plugin/v2/effect/integration"
import { define } from "@klautcode/plugin/v2/effect/plugin"
...
export const KlautcodePlugin = define({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("KlautcodePlugin.load")(function* () {
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Klautcode provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("klautcode", (integration) => {
        integration.name = "Klautcode"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "klautcode", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("klautcode")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("klautcode")
