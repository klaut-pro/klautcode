...
import { ModelsDevPlugin } from "@klautcode/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.KLAUTCODE_MODELS_PATH,
          disabled: Flag.KLAUTCODE_DISABLE_MODELS_FETCH,
        }
        Flag.KLAUTCODE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.KLAUTCODE_MODELS_PATH = previous.path
          Flag.KLAUTCODE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
