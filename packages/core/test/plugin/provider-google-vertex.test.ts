...
import { GoogleVertexPlugin } from "@klautcode/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.klautcode, (provider) => {
          provider.api = {
            ...
            url: "https://code.klaut.pro/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.klautcode))
