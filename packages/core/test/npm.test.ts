...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@klautcode/acme")).toBe("@klautcode/acme")
    expect(Npm.sanitize("@klautcode/acme@1.0.0")).toBe("@klautcode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/klautcode/acme.git"
    const expected = win ? "acme@git+https_//github.com/klautcode/acme.git" : spec
