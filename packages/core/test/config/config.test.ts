...
import { Config } from "@klautcode/core/config"
import { ConfigProvider } from "@klautcode/core/config/provider"
import { AppNodeBuilder } from "@klautcode/core/effect/app-node-builder"
import { LayerNode } from "@klautcode/core/effect/layer-node"
import { ConfigMigrateV1 } from "@klautcode/core/v1/config/migrate"
...
import { Policy } from "@klautcode/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads klautcode JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "klautcode.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "klautcode.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "klautcode.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "klautcode.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "klautcode.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "klautcode.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "klautcode.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .klautcode configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".klautcode"), { recursive: true })
            await fs.mkdir(path.join(directory, ".klautcode"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "klautcode.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "klautcode.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "klautcode.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "klautcode.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "klautcode.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".klautcode", "klautcode.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".klautcode", "klautcode.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".klautcode")),
              AbsolutePath.make(path.join(directory, ".klautcode")),
            ])
