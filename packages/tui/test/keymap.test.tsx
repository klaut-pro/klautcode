...
import { getKlautcodeModeStack, KLAUTCODE_BASE_MODE, KlautcodeKeymapProvider, registerKlautcodeKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerKlautcodeKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when klautcode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getKlautcodeModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getKlautcodeModeStack(keymap).push("autocomplete")
