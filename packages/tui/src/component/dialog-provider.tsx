...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@klautcode/sdk/v2"
...
const CUSTOM_PROVIDER_OPTION_VALUE = "__klautcode_custom_provider__"
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in klautcode.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      description={() =>
        ({
          klautcode: (
            <box gap={1}>
              <text fg={theme.textMuted}>
                Klautcode Zen gives you access to all the best coding models at the cheapest prices with a single API
                key.
              </text>
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://code.klaut.pro/zen</span> to get a key
              </text>
            </box>
          ),
          "klautcode-go": (
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in klautcode.json to use it.`,
          })
          dialog.clear()
          return
        }
