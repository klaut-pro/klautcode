...
export const KLAUTCODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const KLAUTCODE_MODE_KEY = "klautcode.mode"

export const KlautcodeKeymapProvider = KeymapProvider
export const useKlautcodeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type KlautcodeModeStack = ReturnType<typeof createKlautcodeModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, KlautcodeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createKlautcodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(KLAUTCODE_MODE_KEY, KLAUTCODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(KLAUTCODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(KLAUTCODE_MODE_KEY, stack.at(-1)?.mode ?? KLAUTCODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? KLAUTCODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(KLAUTCODE_MODE_KEY, undefined)
...
export function useKlautcodeModeStack() {
  return getKlautcodeModeStack(useKlautcodeKeymap())
}

export function getKlautcodeModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Klautcode mode stack is not registered for this keymap")
  return value
}
...
export function registerKlautcodeKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createKlautcodeModeStack(keymap)
