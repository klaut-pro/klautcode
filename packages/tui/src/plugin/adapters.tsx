import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@klautcode/plugin/tui"
...
import type { useKlautcodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useKlautcodeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getKlautcodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getKlautcodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
