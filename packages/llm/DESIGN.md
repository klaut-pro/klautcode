...
> current private `@klautcode/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@klautcode/ai`
...
- Secondary audience: Klautcode and other durable agent runtimes
...
import { LLM } from "@klautcode/ai"
import { OpenAI } from "@klautcode/ai/providers/openai"
...
```ts
import { Message } from "@klautcode/ai"
...
import { LLM, Tool } from "@klautcode/ai"
...
Klautcode and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@klautcode/ai/promise"
import { OpenAI } from "@klautcode/ai/providers/openai"
...
```ts
import { LLMSchema } from "@klautcode/ai/schema"
...
```ts
import { Provider, Protocol } from "@klautcode/ai/provider"
...
@klautcode/ai/promise
  Promise/AsyncIterable LLM facade

@klautcode/ai/schema
  serializable domain schemas

@klautcode/ai/provider
  experimental Provider and Protocol authoring APIs

@klautcode/ai/providers/openai
@klautcode/ai/providers/anthropic
@klautcode/ai/providers/google
...
```
...
Klautcode should migrate to `generateTurn` / `streamTurn`, preserving its durable
