...
import { createKlautcode, type ToolPart } from "@klautcode/sdk"
...
console.log("🚀 Starting klautcode server...")
const klautcode = await createKlautcode({
  port: 0,
})
console.log("✅ Klautcode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await klautcode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new klautcode session...")
    const { client, server } = klautcode
    ...
    console.log("✅ Created klautcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to klautcode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Klautcode response:", JSON.stringify(result, null, 2))
