import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@klautcode/sdk/v2"
import { SessionTurn } from "@klautcode/session-ui/session-turn"
import { SessionReview } from "@klautcode/session-ui/session-review"
import { DataProvider } from "@klautcode/session-ui/context"
import { FileComponentProvider } from "@klautcode/ui/context/file"
import { WorkerPoolProvider } from "@klautcode/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@klautcode/ui/logo"
import { IconButton } from "@klautcode/ui/icon-button"
import { ProviderIcon } from "@klautcode/ui/provider-icon"
import { iife } from "@klautcode/core/util/iife"
...
import { MessageNav } from "@klautcode/session-ui/message-nav"
import { FileSSR } from "@klautcode/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@klautcode/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/klautcode-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | Klautcode</Title>
              </Show>
              <Meta name="description" content="klautcode - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/klaut-pro/klautcode"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
                              <IconButton
                                as={"a"}
                                href="https://code.klaut.pro/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
