...
import { type WslCommandLine, resolveWslKlautcode, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const klautcode = await resolveWslKlautcode(distro)
  if (!klautcode) throw new Error(nativeT("desktop.wsl.error.klautcodeNotInstalled", { distro }))

  const port = await allocatePort()
  const password = randomUUID()
  const username = "klautcode"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export KLAUTCODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export KLAUTCODE_CLIENT=desktop",
    `export KLAUTCODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export KLAUTCODE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(klautcode)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
