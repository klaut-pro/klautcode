...
let releaseKlautcodeResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectKlautcodeVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectKlautcodeVersion("1.14.35", "1.16.2")).toThrow(
    "Klautcode update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Klautcode", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.klautcode/bin/klautcode",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, klautcodeChecks: {} })
})
...
test("ignores stale background Klautcode checks after removing a WSL server", async () => {
  persistedServers = []
  releaseKlautcodeResolve = undefined
  ...
  await waitFor(() => !!releaseKlautcodeResolve)
  await controller.removeServer("wsl:Debian")
  releaseKlautcodeResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().klautcodeChecks).toEqual({})
})

test("ignores stale startup Klautcode checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking Klautcode", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const klautcode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveKlautcode: async (distro) => {
      klautcode.push(distro)
      return "/home/me/.klautcode/bin/klautcode"
    },
  })
  ...
  expect(klautcode).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(klautcode).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().klautcodeChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check Klautcode in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const klautcode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(klautcode).toEqual(["Debian"])
  expect(Object.keys(controller.getState().klautcodeChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolveKlautcode: async () => {
      await new Promise<void>((resolve) => {
        releaseKlautcodeResolve = resolve
      })
      return "/home/me/.klautcode/bin/klautcode"
    },
  }
