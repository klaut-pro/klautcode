...
const SIDECAR_SERVICE_NAME = "klautcode server"
...
export function preferAppEnv(userDataPath: string) {
  ...
  Object.assign(process.env, {
    ...shellEnv,
    KLAUTCODE_EXPERIMENTAL_ICON_DISCOVERY: "true",
    KLAUTCODE_EXPERIMENTAL_FILEWATCHER: "true",
...
export async function checkHealth(url: string, password?: string | null): Promise<boolean> {
  ...
  if (password) {
    const auth = Buffer.from(`klautcode:${password}`).toString("base64")
