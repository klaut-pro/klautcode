...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.klautcode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `klautcode.settings.dat` is special: it maps to the `klautcode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"klautcode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // klautcode.settings.dat → the electron settings store ("klautcode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "klautcode.global.dat").
  const storeName = filename === "klautcode.settings.dat" ? "klautcode.settings" : filename
