...
This is the living design and status document for `@klautcode/codemode` and its existing V2 Klautcode adapter.
...
### V2 Klautcode adapter

This section describes the `v2` branch integration. On `dev`, CodeMode is integrated through
`packages/klautcode/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
