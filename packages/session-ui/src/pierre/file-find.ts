...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("klautcode-find")
  api.delete("klautcode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("klautcode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("klautcode-find", new Highlight(...rest))
    return true
  }
