...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve klautcode route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "klautcode")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "klautcode")).toBe("openai")
    expect(statProvider("big-pickle", "", "klautcode")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "klautcode"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep klautcode as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "klautcode"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "klautcode"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep klautcode or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "klautcode"), country: "US" })).toMatchObject([
