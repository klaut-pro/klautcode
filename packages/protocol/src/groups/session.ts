...
import { PromptInput } from "@klautcode/schema/prompt-input"
import { Session } from "@klautcode/schema/session"
import { Project } from "@klautcode/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@klautcode/schema/schema"
...
import { SessionEvent } from "@klautcode/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Klautcode process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Klautcode process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
