...
Trim `packages/klautcode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/klautcode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/klautcode`.
- `[x]` `bun typecheck` from `packages/klautcode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/klautcode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/klautcode`.
- `[x]` `bun dev generate > /tmp/klautcode-openapi.json` from `packages/klautcode`.
...
- Edit `packages/klautcode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/klautcode-openapi.json` from `packages/klautcode`.
...
- Check source schemas in files like `packages/klautcode/src/session/schema.ts`, `packages/klautcode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/klautcode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/klautcode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
