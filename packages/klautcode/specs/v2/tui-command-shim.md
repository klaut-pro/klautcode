...
Delete `packages/klautcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/klautcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/klautcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/klautcode`
- TUI plugin loader tests in `packages/klautcode` if runtime plugin API wiring changed
