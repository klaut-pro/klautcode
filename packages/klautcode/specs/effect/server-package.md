...
Practical reference for a future `packages/server` split after the klautcode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/klautcode`.
...
Until enough shared service code lives outside `packages/klautcode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/klautcode`

It should not import `packages/klautcode` services while `packages/klautcode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on klautcode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/klautcode` implementation details.
