...
- Author package entrypoint is `@klautcode/plugin/tui`.
...
```json
{
  "$schema": "https://code.klaut.pro/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/klautcode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@klautcode/plugin/tui`.
- `@klautcode/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@klautcode/plugin/tui"
...
- The value is a semver range checked against the running Klautcode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.klautcode` is absent, no check is performed (backward compatible).
...
- `klautcode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `klautcode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.klautcode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.klautcode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.klautcode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.klautcode` dir or `KLAUTCODE_CONFIG_DIR`, Klautcode installs `@klautcode/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@klautcode/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Klautcode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Klautcode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Klautcode pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@klautcode/plugin/tui`.
...
- `message` is required; `title` defaults to `"klautcode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `klautcode.default`.
...
- Local plugins persist installed themes under the local `.klautcode/themes` area near the plugin config source.
...
- `--pure` / `KLAUTCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.klautcode/plugins/tui-smoke.tsx`
- Local vim plugin: `.klautcode/plugins/tui-vim.tsx`
- Local smoke config: `.klautcode/tui.json`
- Local smoke theme: `.klautcode/plugins/smoke-theme.json`
