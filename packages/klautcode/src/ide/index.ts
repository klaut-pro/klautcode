...
import { IdeEvent } from "@klautcode/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["KLAUTCODE_CALLER"] === "vscode" || process.env["KLAUTCODE_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.klautcode"], {
    nothrow: true,
  })
