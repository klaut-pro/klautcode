...
import type { AssistantMessage, Message, KlautcodeClient, SessionMessageResponse } from "@klautcode/sdk/v2"
...
export const AuthMethodID = "klautcode-login"
...
export class Service extends Context.Service<Service, Interface>()("@klautcode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `klautcode auth login` in the terminal",
      name: "Login with klautcode",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: KlautcodeClient) {
...
function makeUsageService(sdk: KlautcodeClient) {
...
async function loadDirectorySnapshot(sdk: KlautcodeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, klautcode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const klautcodeProvider = providers[ProviderV2.ID.make("klautcode")]
  const klautcodeModel = klautcodeProvider ? Provider.sort(Object.values(klautcodeProvider.models))[0] : undefined
  if (klautcodeProvider && klautcodeModel) return { providerID: klautcodeProvider.id, modelID: klautcodeModel.id }
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "Klautcode prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Klautcode service failure", service })
}
