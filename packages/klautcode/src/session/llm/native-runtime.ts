...
import type { LLMClientShape } from "@klautcode/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("klautcode"))
    return { type: "unsupported", reason: "provider is not openai, klautcode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @klautcode/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains klautcode-owned. The native runtime only adapts
      // the @klautcode/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
