...
const TUI_SCHEMA_URL = "https://code.klaut.pro/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from klautcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const klautcode = await klautcodeFiles(input)
  for (const file of klautcode) {
...
async function klautcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "klautcode"),
    ...(await Filesystem.findUp(["klautcode.json", "klautcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "klautcode"))
  }
  if (Flag.KLAUTCODE_CONFIG) files.push(Flag.KLAUTCODE_CONFIG)
