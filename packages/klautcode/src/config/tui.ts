...
import { isRecord } from "@klautcode/tui/util/record"
...
import { TuiKeybind } from "@klautcode/tui/config/keybind"
...
import { TuiConfig } from "@klautcode/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@klautcode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old klautcode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.klautcode`
  // folders between cwd and home, and KLAUTCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.KLAUTCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit KLAUTCODE_TUI_CONFIG override, if set.
  if (Flag.KLAUTCODE_TUI_CONFIG) {
    const configFile = Flag.KLAUTCODE_TUI_CONFIG
  ...
  // 4. `.klautcode` directories (and KLAUTCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".klautcode") || dir === Flag.KLAUTCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".klautcode") && dir !== Flag.KLAUTCODE_CONFIG_DIR) continue
