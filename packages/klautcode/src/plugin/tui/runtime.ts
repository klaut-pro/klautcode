...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@klautcode/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@klautcode/core/effect/app-node-builder"
import { errorData, errorMessage } from "@klautcode/tui/util/error"
...
import { hasTheme, upsertTheme } from "@klautcode/tui/context/theme"
import { Global } from "@klautcode/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@klautcode/core/util/flock"
import { Flag } from "@klautcode/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@klautcode/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@klautcode/core/v1/config/plugin"
import { createCommandShim } from "@klautcode/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@klautcode/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".klautcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".klautcode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".klautcode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.KLAUTCODE_PURE ? [] : pluginOrigins
    if (Flag.KLAUTCODE_PURE && pluginOrigins.length) {
    }
