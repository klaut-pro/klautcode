...
export const DEPRECATED_PLUGIN_PACKAGES = ["klautcode-openai-codex-auth", "klautcode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, klautcodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(klautcodeVersion) || semver.major(klautcodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.klautcode
  if (typeof range !== "string") return
  if (!semver.satisfies(klautcodeVersion, range)) {
    throw new Error(`Plugin requires klautcode ${range} but running ${klautcodeVersion}`)
  }
}
