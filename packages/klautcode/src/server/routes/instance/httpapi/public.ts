...
import { KlautcodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for klautcode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(KlautcodeHttpApi)
...
export const PublicApi = KlautcodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "klautcode",
    version: "1.0.0",
    description: "klautcode api",
