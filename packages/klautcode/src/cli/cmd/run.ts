import type { PermissionV1 } from "@klautcode/core/v1/permission"
import { FSUtil } from "@klautcode/core/fs-util"
// CLI entry point for `klautcode run` and `klautcode --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`klautcode --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`klautcode --mini --attach`): connects to a running
//      klautcode server and runs interactive mode against it.
//
...
import { createKlautcodeClient, type KlautcodeClient, type ToolPart } from "@klautcode/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<KlautcodeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run klautcode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running klautcode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to KLAUTCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to KLAUTCODE_SERVER_USERNAME or 'klautcode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: KlautcodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: KlautcodeClient, sessionID: string) {
      ...
      async function current(sdk: KlautcodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: KlautcodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: KlautcodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: KlautcodeClient) {
        ...
        async function loop(client: KlautcodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createKlautcodeClient({
        baseUrl: "http://klautcode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
