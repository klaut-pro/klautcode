// Subprocess test harness for the klautcode CLI. Spawns the real binary against
...
//
// Configuration flows through klautcode's built-in test affordances:
//   - KLAUTCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - KLAUTCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - KLAUTCODE_DISABLE_PROJECT_CONFIG : skip walking up for klautcode.json
//   - KLAUTCODE_PURE                : skip external plugin discovery + install
//   - KLAUTCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `klautcode.run` is fully wired. The shape supports adding more
// builders (`klautcode.serve(opts)`, `klautcode.acp(opts)`, `klautcode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of KlautcodeCli.
...
import { AppProcess } from "@klautcode/core/process"
...
const klautcodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(klautcodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    KLAUTCODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    KLAUTCODE_CONFIG_CONTENT: configJson,
    KLAUTCODE_DISABLE_PROJECT_CONFIG: "1",
    KLAUTCODE_PURE: "1",
    KLAUTCODE_DISABLE_AUTOUPDATE: "1",
    KLAUTCODE_DISABLE_AUTOCOMPACT: "1",
    KLAUTCODE_DISABLE_MODELS_FETCH: "1",
    KLAUTCODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `klautcode run`. New flags should
...
// `klautcode serve` is a long-lived process — it never exits on its own.
...
// `klautcode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type KlautcodeCli = {
  ...
  // Spawn `klautcode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `klautcode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly klautcode: KlautcodeCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("klautcode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          KLAUTCODE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("klautcode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("klautcode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "klautcode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `klautcode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("klautcode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const klautcode: KlautcodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, klautcode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `klautcode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "klautcode") {
...
// every call site. Use this for any test that needs the klautcode CLI fixture.
//
...
// resources (e.g. `klautcode.serve`) without an extra `Effect.scoped` wrapper —
