    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".klautcode/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const klautcodeModel = {
      ...openaiModel,
      providerID: "klautcode",
      api: {
        id: "klautcode-test",
        url: "https://api.code.klaut.pro",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, klautcodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.klautcode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.klautcode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          klautcode: { itemId: "msg_klautcode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              klautcode: { itemId: "msg_klautcode_part" },
    ...
    expect(result[0].providerOptions?.klautcode?.itemId).toBe("msg_klautcode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.klautcode?.itemId).toBe("msg_klautcode_part")
