...
import { LLMEvent, Usage } from "@klautcode/llm"
...
describe("util.token.estimate", () => {
  test("estimates tokens from text (3 chars per token, ceil)", () => {
    const text = "x".repeat(4000)
    expect(Token.estimate(text)).toBe(Math.ceil(4000 / 3))
  })

  test("estimates tokens from larger text", () => {
    const text = "y".repeat(20_000)
    expect(Token.estimate(text)).toBe(Math.ceil(20_000 / 3))
  })
