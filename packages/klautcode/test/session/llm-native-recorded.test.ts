...
import { ModelsDev } from "@klautcode/core/models-dev"
import { HttpRecorder } from "@klautcode/http-recorder"
import { HttpRecorderInternal } from "@klautcode/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@klautcode/llm"
import { RequestExecutor } from "@klautcode/llm/route"
...
const zenURL = (connection: string) => `https://console.code.klaut.pro/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.KLAUTCODE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["klautcode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("KLAUTCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("KLAUTCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["klautcode", "native", "oauth", "tool-loop"],
  ...
  {
    id: "klautcode-proxy",
    name: "Klautcode proxy",
    providerID: ProviderV2.ID.klautcode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["klautcode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.KLAUTCODE_RECORD_CONSOLE_TOKEN && process.env.KLAUTCODE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.klautcode,
        name: "Klautcode Zen",
        env: ["KLAUTCODE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.KLAUTCODE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.KLAUTCODE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.KLAUTCODE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["klautcode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("KLAUTCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("KLAUTCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("KLAUTCODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set KLAUTCODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the klautcode LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "klautcode.json"),
      JSON.stringify({ $schema: "https://code.klaut.pro/config.json", ...scenario.config(model) }),
    ),
  )
