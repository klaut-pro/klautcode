...
it.instance("plan agent denies edits except .klautcode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".klautcode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".klautcode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.KLAUTCODE_TEST_HOME
      process.env.KLAUTCODE_TEST_HOME = test.directory
