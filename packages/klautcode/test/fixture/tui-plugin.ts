...
import type { HostPluginApi } from "@klautcode/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createKlautcodeClient({
  ...
  let selected = opts.theme?.selected ?? "klautcode"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "klautcode.default",
