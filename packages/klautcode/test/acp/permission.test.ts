...
import type { Event, KlautcodeClient } from "@klautcode/sdk/v2"
...
type PermissionReplyParams = Parameters<KlautcodeClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "klautcode-acp-permission-"))
