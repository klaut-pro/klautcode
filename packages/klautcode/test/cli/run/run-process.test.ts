// Subprocess integration tests for `klautcode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `klautcode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `KLAUTCODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("klautcode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, klautcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* klautcode.run("say hi")
        klautcode.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, klautcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* klautcode.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, klautcode }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* klautcode.run("think", { extraArgs: ["--thinking"] })
        klautcode.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* klautcode.run("think again")
        klautcode.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, klautcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* klautcode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, klautcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* klautcode.run("say hi", { format: "json" })
        klautcode.expectExit(result, 0)

        const events = klautcode.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ klautcode }) =>
      Effect.gen(function* () {
        const result = yield* klautcode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, klautcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* klautcode.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, klautcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* klautcode.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, klautcode }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* klautcode.run("request permission", { permission: { bash: "ask" } })
        klautcode.expectExit(denied, 0)
        ...
        const allowed = yield* klautcode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        klautcode.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* klautcode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        klautcode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, klautcode }) =>
      Effect.gen(function* () {
        ...
        const server = yield* klautcode.serve()

        const result = yield* klautcode.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, klautcode }) =>
      Effect.gen(function* () {
        const result = yield* klautcode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, klautcode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* klautcode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
