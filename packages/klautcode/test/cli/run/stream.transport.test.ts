...
import { KlautcodeClient, type GlobalEvent } from "@klautcode/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<KlautcodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<KlautcodeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<KlautcodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<KlautcodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<KlautcodeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://klautcode.test"),
...
function sdk(
  input: {
    ...
    subscribe?: KlautcodeClient["event"]["subscribe"]
    globalEvent?: KlautcodeClient["global"]["event"]
    promptAsync?: KlautcodeClient["session"]["promptAsync"]
    status?: KlautcodeClient["session"]["status"]
    messages?: KlautcodeClient["session"]["messages"]
    children?: KlautcodeClient["session"]["children"]
    permissions?: KlautcodeClient["permission"]["list"]
    questions?: KlautcodeClient["question"]["list"]
  } = {},
) {
  const client = new KlautcodeClient()

  const subscribe: KlautcodeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: KlautcodeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: KlautcodeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: KlautcodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: KlautcodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: KlautcodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: KlautcodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: KlautcodeClient["question"]["list"] = input.questions ?? (() => ok([]))
