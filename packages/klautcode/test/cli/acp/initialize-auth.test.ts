...
describe("klautcode acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ klautcode }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ klautcode }))
        ...
        expect(initialized.agentInfo?.name).toBe("Klautcode")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ klautcode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ klautcode })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("klautcode-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "klautcode-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.KLAUTCODE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ klautcode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("klautcode-login")
