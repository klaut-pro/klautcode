...
import { EditorContextProvider, useEditorContext, type EditorIntegration } from "@klautcode/tui/context/editor"
import { tmpdir } from "../../fixture/fixture"
import { FakeWebSocket } from "../../lib/websocket"
import { TestTuiContexts } from "../../fixture/tui-environment"
import { discoverEditorConnection } from "@klautcode/tui/editor"

const originalClaudePort = process.env.CLAUDE_CODE_SSE_PORT
const originalKlautcodePort = process.env.KLAUTCODE_EDITOR_SSE_PORT

afterEach(() => {
  process.env.CLAUDE_CODE_SSE_PORT = originalClaudePort
  process.env.KLAUTCODE_EDITOR_SSE_PORT = originalKlautcodePort
})
...
function mountEditorContext(WebSocketImpl?: typeof WebSocket) {
  ...
  createRoot((nextDispose) => {
    ...
    const value = process.env.CLAUDE_CODE_SSE_PORT || process.env.KLAUTCODE_EDITOR_SSE_PORT
    return (
...
test("useEditorContext reconnect switches editor server by session directory", async () => {
  ...
  process.env.KLAUTCODE_EDITOR_SSE_PORT = undefined
...
test("useEditorContext connects with KLAUTCODE_EDITOR_SSE_PORT", async () => {
  await using tmp = await tmpdir()
  process.env.CLAUDE_CODE_SSE_PORT = undefined
  process.env.KLAUTCODE_EDITOR_SSE_PORT = "4020"
