...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ klautcode }) =>
    Effect.gen(function* () {
      const result = yield* klautcode.spawn(["--replay-limit", "10"])

      klautcode.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ klautcode }) =>
    Effect.gen(function* () {
      const result = yield* klautcode.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ klautcode }) =>
    Effect.gen(function* () {
      const result = yield* klautcode.spawn(["--mini", "--port", "4096"])
