...
describe("klautcode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // KLAUTCODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ klautcode }) =>
      Effect.gen(function* () {
        const r = yield* klautcode.spawn(["mcp", "list"])
        klautcode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ klautcode }) =>
      Effect.gen(function* () {
        const r = yield* klautcode.spawn(["providers", "list"])
        klautcode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ klautcode }) =>
      Effect.gen(function* () {
        const r = yield* klautcode.spawn(["models"])
        klautcode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ klautcode }) =>
      Effect.gen(function* () {
        const r = yield* klautcode.spawn(["agent", "list"])
        klautcode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh KLAUTCODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ klautcode }) =>
      Effect.gen(function* () {
        const r = yield* klautcode.spawn(["session", "list"])
        klautcode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ klautcode }) =>
      Effect.gen(function* () {
        const r = yield* klautcode.spawn(["stats"])
        klautcode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under KLAUTCODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ klautcode }) =>
      Effect.gen(function* () {
        const r = yield* klautcode.spawn(["db", "path"])
        klautcode.expectExit(r, 0, "db path")
