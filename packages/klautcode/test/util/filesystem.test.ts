...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "klautcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "klautcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "klautcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "klautcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["klautcode.json", "klautcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "klautcode.json"),
        path.join(project, "klautcode.jsonc"),
      ])
    })
  })
