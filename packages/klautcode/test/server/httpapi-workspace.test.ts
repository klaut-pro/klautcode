...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-klautcode-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-klautcode-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://code.klaut.pro/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-klautcode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-klautcode-workspace")
