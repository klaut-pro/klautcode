...
const stateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      KLAUTCODE_EXPERIMENTAL_WORKSPACES: Flag.KLAUTCODE_EXPERIMENTAL_WORKSPACES,
    }
    ...
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.KLAUTCODE_EXPERIMENTAL_WORKSPACES = original.KLAUTCODE_EXPERIMENTAL_WORKSPACES
        await resetDatabase()
      }),
    )
  }),
