...
Pour déboguer les problèmes avec Klautcode, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\klautcode\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `klautcode --log-level DEBUG`.

---

## Stockage

klautcode stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/klautcode/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\klautcode`
...
Klautcode Desktop exécute un serveur Klautcode local (le side-car `klautcode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Klautcode` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/klautcode/klautcode.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\klautcode\klautcode.jsonc`
...
Klautcode peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/klautcode/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\klautcode\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Klautcode puisse le reconstruire.

1. Quittez complètement Klautcode Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/klautcode`
- **Linux** : supprimez `~/.cache/klautcode` (ou exécutez `rm -rf ~/.cache/klautcode`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\klautcode`

3. Redémarrez le bureau Klautcode.

---

### Résoudre les problèmes de connexion au serveur

Klautcode Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `klautcode.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `KLAUTCODE_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `KLAUTCODE_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Klautcode Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Klautcode.

---

### Les notifications ne s'affichent pas

Klautcode Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Klautcode dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Klautcode.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Klautcode Desktop) :

- `klautcode.settings.dat` (serveur de bureau par défaut URL)
- `klautcode.global.dat` et `klautcode.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Klautcode :
...
3. Assurez-vous d'avoir la dernière version avec `klautcode upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `klautcode models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\klautcode`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. klautcode installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\klautcode`

2. Redémarrez klautcode pour réinstaller les derniers packages du fournisseur

Cela forcera klautcode à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
klautcode détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
