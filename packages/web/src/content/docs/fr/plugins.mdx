---
title: Plugins
description: Écrivez vos propres plugins pour étendre Klautcode.
---

Les plugins vous permettent d'étendre Klautcode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Klautcode.
...
- `.klautcode/plugins/` - Plugins au niveau du projet
- `~/.config/klautcode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/klautcode/node_modules/`.
...
1. Configuration globale (`~/.config/klautcode/klautcode.json`)
2. Configuration du projet (`klautcode.json`)
3. Répertoire global des plugins (`~/.config/klautcode/plugins/`)
4. Répertoire des plugins du projet (`.klautcode/plugins/`)
...
Klautcode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK klautcode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre klautcode.
...
:::note
Si vous utilisez l'application de bureau Klautcode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez klautcode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à klautcode :
...
L'assistant `tool` crée un outil personnalisé que klautcode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour klautcode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://code.klaut.pro/docs/sdk) pour plus de détails.
