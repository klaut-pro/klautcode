---
title: SDK
description: Client JS de type sécurisé pour le serveur klautcode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK klautcode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler klautcode par programme.
...
Créez une instance de klautcode :

```javascript
import { createKlautcode } from "@klautcode/sdk"

const { client } = await createKlautcode()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `klautcode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${klautcode.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de klautcode, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@klautcode/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*klautcode" },
})
