...
Avec Klautcode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Klautcode utilise notre propre thème `klautcode`.
...
Klautcode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Klautcode corresponde à l'apparence de leur terminal
...
Klautcode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/klautcode/themes/*.json` ou `$XDG_CONFIG_HOME/klautcode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.klautcode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.klautcode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/klautcode/themes
vim ~/.config/klautcode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .klautcode/themes
vim .klautcode/themes/my-theme.json
```
...
{
  "$schema": "https://code.klaut.pro/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
