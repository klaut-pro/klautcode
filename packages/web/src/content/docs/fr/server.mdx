---
title: Serveur
description: Interagissez avec le serveur klautcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `klautcode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client klautcode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `klautcode.local` |
...
Définissez `KLAUTCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `klautcode`, ou définissez `KLAUTCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `klautcode serve` et à `klautcode web`.
...
Lorsque vous exécutez `klautcode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur klautcode pour interagir avec klautcode par programmation.
:::

Cette architecture permet à klautcode de prendre en charge plusieurs clients et vous permet d'interagir avec klautcode par programme.

Vous pouvez exécuter `klautcode serve` pour démarrer un serveur autonome. Si vous avez le
klautcode TUI en cours d'exécution, `klautcode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Klautcode [IDE](/docs/ide).
...
Le serveur klautcode expose les API suivantes.
