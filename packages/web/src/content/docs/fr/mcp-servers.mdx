...
Vous pouvez ajouter des outils externes à Klautcode à l'aide du _Model Context Protocol_ ou MCP. Klautcode prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [Klautcode Config](https://code.klaut.pro/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/klautcode`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
Klautcode gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, Klautcode :
...
Si le serveur nécessite une authentification, Klautcode vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `klautcode mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, Klautcode stockera les jetons en toute sécurité dans `~/.local/share/klautcode/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans Klautcode, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration Klautcode comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter Klautcode à votre compte Sentry.
