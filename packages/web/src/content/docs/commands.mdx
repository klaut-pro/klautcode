...
Create `.klautcode/commands/test.md`:
...
You can add custom commands through the Klautcode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Klautcode [config](/docs/config):
...
- Per-project: `.klautcode/commands/`
...
klautcode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
