---
title: TUI
description: Using the Klautcode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Klautcode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Klautcode starts the TUI for the current directory.
...
When using the Klautcode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Klautcode. Allows you to select from available providers and add their API keys.
...
Exit Klautcode. _Aliases_: `/quit`, `/q`
...
This is separate from `klautcode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Klautcode waits after the leader key. Defaults to `2000`.
...
Use `KLAUTCODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `klautcode.default`.
