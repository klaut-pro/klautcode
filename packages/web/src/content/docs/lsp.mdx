---
title: LSP Servers
description: Klautcode integrates with your LSP servers.
---

Klautcode can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

Klautcode comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `KLAUTCODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and klautcode opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your klautcode config.
...
```json title="klautcode.json" {5-13}
{
  "$schema": "https://code.klaut.pro/config.json",
  "lsp": {
    "custom-lsp": {
