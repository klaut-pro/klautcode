---
title: Server
description: Interact with klautcode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `klautcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an klautcode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `klautcode.local` |
...
Set `KLAUTCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `klautcode`, or set `KLAUTCODE_SERVER_USERNAME` to override it. This applies to both `klautcode serve` and `klautcode web`.
...
When you run `klautcode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the klautcode server to interact with klautcode programmatically.
:::

This architecture lets klautcode support multiple clients and allows you to interact with klautcode programmatically.

You can run `klautcode serve` to start a standalone server. If you have the
klautcode TUI running, `klautcode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Klautcode [IDE](/docs/ide) plugins.
...
The klautcode server exposes the following APIs.
