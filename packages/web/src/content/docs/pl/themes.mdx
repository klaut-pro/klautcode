...
Dzięki klautcode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie klautcode używa naszego własnego motywu `klautcode`.
...
klautcode ma kilka wbudowanych motywów.
...
- Chcą, aby Klautcode pasował do wyglądu ich terminala
...
klautcode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/klautcode/themes/*.json` lub `$XDG_CONFIG_HOME/klautcode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.klautcode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.klautcode/themes/*.json`
