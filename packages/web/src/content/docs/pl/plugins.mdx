---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć klautcode.
---

Wtyczki umożliwiające wyłączenie klautcode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania klautcode.
...
- `.klautcode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/klautcode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/klautcode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/klautcode/klautcode.json`)
2. Konfiguracja projektu (`klautcode.json`)
3. Globalny katalog wtyczek (`~/.config/klautcode/plugins/`)
4. Katalog wtyczek projektu (`.klautcode/plugins/`)
...
klautcode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z klautcode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do klautcode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej klautcode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj klautcode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do klautcode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod klautcode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://code.klaut.pro/docs/sdk).
