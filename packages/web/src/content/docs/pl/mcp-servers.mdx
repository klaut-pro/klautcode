...
Możesz dodać dodatkowe informacje do klautcode za pomocą _Model Context Protocol_, w skrócie MCP. klautcode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [klautcode Config](https://code.klaut.pro/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/klautcode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
klautcode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, klautcode:
...
Jeśli serwer wymaga uwierzytelnienia, klautcode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `klautcode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji klautcode bezpieczne przechowa tokeny w `~/.local/share/klautcode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w klautcode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie klautcode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć klautcode z kontem Sentry.
