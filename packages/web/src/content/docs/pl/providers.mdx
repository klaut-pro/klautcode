---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w klautcode.
---

import config from "../../../../config.mjs"
export const console = config.console

klautcode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji klautcode.
...
w `~/.local/share/klautcode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim klautcode
config.
...
Klautcode Zen to lista modeli dostarczonych przez zespół klautcode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z klautcode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Klautcode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Klautcode Zen` i przejdź do [code.klaut.pro/auth](https://code.klaut.pro/zen).
...
Działa jak każdy inny dostawca w klautcode i jest całkowicie opcjonalny w użyciu.

---

## Klautcode Go

Klautcode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół klautcode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z klautcode.

1. Uruchom polecenie `/connect` w TUI, wybierz `Klautcode Go` i przejdź do [code.klaut.pro/auth](https://code.klaut.pro/zen).
...
Aby używać Amazon Bedrock z klautcode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania klautcode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `klautcode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w klautcode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować klautcode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod klautcode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji klautcode.
...
klautcode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować klautcode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`klautcode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem klautcode:
...
Aby używać Google Vertex AI z klautcode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania klautcode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą klautcode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [klautcode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację klautcode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja klautcode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować klautcode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować klautcode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować klautcode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem klautcode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/klautcode).
:::
...
Aby korzystać z Ollama Cloud z klautcode:
...
3. Skopiuj klucz API do użycia w klautcode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w klautcode musisz pobrać informacje o modelu lokalnie:
...
Klautcode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół klautcode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Klautcode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Klautcode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API klautcode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji klautcode.
...
5. Możesz je także dostosować za pomocą konfiguracji klautcode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z klautcode:
...
Możesz także dostosować modele za pomocą konfiguracji klautcode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z klautcode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `klautcode.json` w katalogu projektu:
...
Pola `limit` pozwalają klautcode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `klautcode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację klautcode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji klautcode.
