...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Klautcode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Klautcode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `KLAUTCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `KLAUTCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Klautcode lub gdy zmienna środowiskowa `KLAUTCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Klautcode:
