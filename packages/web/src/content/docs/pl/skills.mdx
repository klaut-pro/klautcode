...
Umiejętności agenta pozwalają klautcode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
klautcode przeszukuje te lokalizacje:

- Project config: `.klautcode/skills/<name>/SKILL.md`
- Global config: `~/.config/klautcode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu klautcode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.klautcode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/klautcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.klautcode/skills/git-release/SKILL.md` w ten sposób:
...
klautcode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `klautcode.json`:
...
**Dla agentów wbudowanych** (w `klautcode.json`):
