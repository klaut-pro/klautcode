---
title: Serwer
description: Interakcja z serwerem klautcode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `klautcode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient klautcode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `klautcode.local` |
...
Ustaw `KLAUTCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `klautcode` lub ustaw `KLAUTCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `klautcode serve`, jak i `klautcode web`.
...
Kiedy uruchomisz `klautcode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera klautcode do programowej interakcji z klautcode.
:::

Ta architektura umożliwia obsługę wielu klientów przez klautcode i programową interakcję z kodem otwartym.

Możesz uruchomić `klautcode serve`, aby uruchomić samodzielny serwer. Jeśli masz
klautcode TUI działa, `klautcode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki klautcode [IDE](/docs/ide).
...
Serwer klautcode udostępnia następujące interfejsy API.
