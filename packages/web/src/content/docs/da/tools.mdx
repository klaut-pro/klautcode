...
Her er alle de innebygde verktøyene som er tilgængelige i Klautcode.
...
:::note
Dette verktøyet er bare tilgængelig når `KLAUTCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `KLAUTCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Klautcode-leverandøren eller når miljøvariabelen `KLAUTCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
