...
Agentferdigheter lar Klautcode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Klautcode søker etter disse stedene:

- Prosjektkonfigurasjon: `.klautcode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/klautcode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Klautcode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.klautcode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/klautcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.klautcode/skills/git-release/SKILL.md` slik:
...
Klautcode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `klautcode.json`:
...
**For innebygde agenter** (i `klautcode.json`):
