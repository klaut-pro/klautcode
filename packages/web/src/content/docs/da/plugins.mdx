---
title: Plugins
description: Skriv dine egne plugins for at udvide Klautcode.
---

Plugins giver dig mulighed for at udvide Klautcode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Klautcodes standardadfærd.
...
- `.klautcode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/klautcode/node_modules/`.
...
1. Global konfiguration (`~/.config/klautcode/klautcode.json`)
2. Projektkonfiguration (`klautcode.json`)
3. Global plugin-mappe (`~/.config/klautcode/plugins/`)
4. Projekt plugin bibliotek (`.klautcode/plugins/`)
...
Klautcode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En klautcode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide klautcode.
...
:::note
Hvis du bruger Klautcode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå klautcode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til klautcode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som klautcode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for klautcode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://code.klaut.pro/docs/sdk) for detaljer.
