---
title: Server
description: Interager med klautcode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `klautcode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en klautcode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `klautcode.local` |
...
Indstil `KLAUTCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `klautcode`, eller indstil `KLAUTCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `klautcode serve` og `klautcode web`.
...
Når du kører `klautcode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug klautcode-serveren til at interagere med klautcode programmatisk.
:::

Denne arkitektur lader klautcode understøtte flere klienter og lader dig interagere med klautcode programmatisk.

Du kan køre `klautcode serve` for at starte en selvstændig server. Hvis du har
klautcode TUI kørende, vil `klautcode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Klautcode [IDE](/docs/ide) plugins.
...
Klautcode-serveren viser følgende API'er.
