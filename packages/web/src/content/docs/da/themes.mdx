...
Med Klautcode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Klautcode vårt eget `klautcode`-tema.
...
Klautcode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/klautcode/themes/*.json` eller `$XDG_CONFIG_HOME/klautcode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.klautcode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.klautcode/themes/*.json`
