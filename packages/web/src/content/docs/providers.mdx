---
title: Providers
description: Using any LLM provider in Klautcode.
---

import config from "../../../config.mjs"
export const console = config.console

Klautcode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Klautcode config.
...
in `~/.local/share/klautcode/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Klautcode
config.
...
Klautcode Zen is a list of models provided by the Klautcode team that have been
tested and verified to work well with Klautcode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Klautcode Zen.
:::

1. Run the `/connect` command in the TUI, select `Klautcode Zen`, and head to [code.klaut.pro/auth](https://code.klaut.pro/zen).
...
It works like any other provider in Klautcode and is completely optional to use.

---

## Klautcode Go

Klautcode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Klautcode team that have been
tested and verified to work well with Klautcode.

1. Run the `/connect` command in the TUI, select `Klautcode Go`, and head to [code.klaut.pro/auth](https://code.klaut.pro/zen).
...
To use Amazon Bedrock with Klautcode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running klautcode:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY klautcode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile klautcode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX klautcode
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `klautcode.json`:
...
Klautcode. Anthropic explicitly prohibits this.

Previous versions of Klautcode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Klautcode with zero setup:
...
You can configure klautcode to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="klautcode.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://code.klaut.pro/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for klautcode to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX klautcode
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX klautcode
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your klautcode config.
...
Klautcode supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Klautcode uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Klautcode. Sign in and approve.

   :::note
   Klautcode requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Klautcode surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Klautcode forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Klautcode is experimental. Features, configuration, and
behavior may change in future releases.
:::

Klautcode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in klautcode auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Klautcode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Klautcode
to only use your own GitLab-hosted instance, add the following to your
`klautcode.json` file. It is also recommended to disable session sharing.
...
Customize through `klautcode.json`:
...
When a `duo-workflow-*` model is selected, Klautcode will:
...
4. Route tool execution requests through Klautcode's permission-gated tool system
...
{
  "$schema": "https://code.klaut.pro/config.json",
  "plugin": ["klautcode-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with klautcode:
...
To use GMI Cloud with Klautcode:
...
To use Google Vertex AI with Klautcode:
...
2. Set the required environment variables:
   ...
   Set them while running klautcode.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id klautcode
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through klautcode, you can always configure it yourself.
...
```jsonc title="~/.config/klautcode/klautcode.jsonc"
{
  "$schema": "https://code.klaut.pro/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [klautcode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Klautcode conversation as a session in Helicone.

```bash
npm install -g klautcode-helicone-session
```
...
```json title="klautcode.json"
{
  "plugin": ["klautcode-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Klautcode conversation listed as a separate session.
...
You can configure klautcode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="klautcode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://code.klaut.pro/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure klautcode to use local models through LM Studio.

```json title="klautcode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://code.klaut.pro/config.json",
  "provider": {
    "lmstudio": {
...
You can configure klautcode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Klautcode. See the [Ollama integration docs](https://docs.ollama.com/integrations/klautcode) for details.
:::

```json title="klautcode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://code.klaut.pro/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Klautcode:
...
3. Copy the API key for use in Klautcode.
...
6. **Important**: Before using cloud models in Klautcode, you must pull the model information locally:
...
Klautcode Zen is a list of tested and verified models provided by the Klautcode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Klautcode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Klautcode Zen**.

   ```txt
   /connect
   ```

3. Enter your Klautcode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your klautcode config.
...
5. You can also customize them through your klautcode config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' klautcode
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group klautcode
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Klautcode:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=klautcode&utm_medium=docs&utm_campaign=cortex-provider).

klautcode's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. Klautcode captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting klautcode:
...
The model catalog is provided automatically. A minimal `klautcode.json` is all that's needed:
...
You can also customize models through your klautcode config. Here's an example of specifying provider routing order.
...
2. Select **SuperGrok Subscription**. Klautcode opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by Klautcode. Klautcode polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
Klautcode refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
...
:::tip
You can use any OpenAI-compatible provider with klautcode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in klautcode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `klautcode.json` file in your project directory:

   ```json title="klautcode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://code.klaut.pro/config.json",
     "provider": {
       "myprovider": {
...
```json title="klautcode.json" {9,11,17-20}
{
  "$schema": "https://code.klaut.pro/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Klautcode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `klautcode auth list` to see if the credentials
...
2. For custom providers, check the klautcode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your klautcode config.
