---
title: サーバー
description: HTTP 経由で Klautcode サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`klautcode serve` コマンドは、Klautcode クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `klautcode.local` |
...
HTTP 基本認証でサーバーを保護するには、`KLAUTCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `klautcode` になるか、`KLAUTCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`klautcode serve` と `klautcode web` の両方に当てはまります。
...
`klautcode` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Klautcode サーバーを使用して、プログラムで Klautcode と対話します。
:::

`klautcode serve` を実行してスタンドアロンサーバーを起動できます。すでに Klautcode TUI を実行している場合でも、`klautcode serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Klautcode [IDE](/docs/ide) プラグインによって使用されます。
