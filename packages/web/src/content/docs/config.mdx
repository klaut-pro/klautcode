---
title: Config
description: Using the Klautcode JSON config.
---

You can configure Klautcode using a JSON config file.

---

## Format

Klautcode supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="klautcode.jsonc"
{
...
1. **Remote config** (from `.well-known/klautcode`) - organizational defaults
2. **Global config** (`~/.config/klautcode/klautcode.json`) - user preferences
3. **Custom config** (`KLAUTCODE_CONFIG` env var) - custom overrides
4. **Project config** (`klautcode.json` in project) - project-specific settings
5. **`.klautcode` directories** - agents, commands, plugins
6. **Inline config** (`KLAUTCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/klautcode/` on macOS) - admin-controlled
...
:::note
The `.klautcode` and `~/.config/klautcode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/klautcode` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/klautcode"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Klautcode config in `~/.config/klautcode/klautcode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/klautcode/tui.json`.
...
Add `klautcode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Klautcode starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `KLAUTCODE_CONFIG` environment variable.

```bash
export KLAUTCODE_CONFIG=/path/to/my/custom-config.json
klautcode run "Hello world"
```
...
Specify a custom config directory using the `KLAUTCODE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.klautcode` directory, and should
follow the same structure.

```bash
export KLAUTCODE_CONFIG_DIR=/path/to/my/config-directory
klautcode run "Hello world"
```

The custom directory is loaded after the global config and `.klautcode` directories, so it **can override** their settings.
...
Drop an `klautcode.json` or `klautcode.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/klautcode/` |
| Linux    | `/etc/klautcode/`                         |
| Windows  | `%ProgramData%\klautcode`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Klautcode reads managed preferences from the `ai.klautcode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Klautcode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.klautcode.managed.plist`
2. `/Library/Managed Preferences/ai.klautcode.managed.plist`

The plist keys map directly to `klautcode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.klautcode.managed` PayloadType. The Klautcode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.klautcode.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.klautcode.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.klautcode</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`code.klaut.pro/config.json`**](https://code.klaut.pro/config.json).

TUI config uses [**`code.klaut.pro/tui.json`**](https://code.klaut.pro/tui.json).
...
Use `KLAUTCODE_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `klautcode.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `klautcode serve` and `klautcode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Klautcode server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `klautcode.local`. Useful for running multiple instances on the same network.
...
If not specified, Klautcode will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Klautcode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Klautcode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Klautcode actions on configured resources. Currently, policies can control which providers Klautcode may use.
...
Klautcode normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Klautcode omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/klautcode/agents/` or `.klautcode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Klautcode will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`klautcode run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/klautcode/commands/` or `.klautcode/commands/`. [Learn more here](/docs/commands).
...
Klautcode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Klautcode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, klautcode **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Klautcode with custom tools, hooks, and integrations.

Place plugin files in `.klautcode/plugins/` or `~/.config/klautcode/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="klautcode.json"
{
  "$schema": "https://code.klaut.pro/config.json",
  "plugin": ["klautcode-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict Klautcode to only use specific providers rather than disabling them one by one.
