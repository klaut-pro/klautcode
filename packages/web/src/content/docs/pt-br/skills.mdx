...
As habilidades do agente permitem que o klautcode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O klautcode pesquisa nesses locais:

- Configuração do projeto: `.klautcode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/klautcode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o klautcode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.klautcode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/klautcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.klautcode/skills/git-release/SKILL.md` assim:
...
O klautcode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `klautcode.json`:
...
**Para agentes embutidos** (em `klautcode.json`):
