...
Com o klautcode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o klautcode usa nosso próprio tema `klautcode`.
...
O klautcode vem com vários temas embutidos.
...
- Querem que o klautcode corresponda à aparência do seu terminal
...
O klautcode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/klautcode/themes/*.json` ou `$XDG_CONFIG_HOME/klautcode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.klautcode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.klautcode/themes/*.json`
