---
title: Servidor
description: Interaja com o servidor klautcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `klautcode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente klautcode pode usar.
...
```bash
klautcode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `klautcode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
klautcode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `KLAUTCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `klautcode`, ou defina `KLAUTCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `klautcode serve` quanto ao `klautcode web`.

```bash
KLAUTCODE_SERVER_PASSWORD=your-password klautcode serve
```
...
Quando você executa `klautcode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor klautcode para interagir com o klautcode programaticamente.
:::

Essa arquitetura permite que o klautcode suporte múltiplos clientes e permite que você interaja com o klautcode programaticamente.

Você pode executar `klautcode serve` para iniciar um servidor autônomo. Se você tiver o TUI do klautcode em execução, `klautcode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do klautcode [IDE](/docs/ide).
...
O servidor klautcode expõe as seguintes APIs.
