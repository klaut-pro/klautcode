---
title: Plugins
description: Escreva seus próprios plugins para estender o klautcode.
---

Plugins permitem que você estenda o klautcode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do klautcode.
...
- `.klautcode/plugins/` - Plugins em nível de projeto
- `~/.config/klautcode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/klautcode/node_modules/`.
...
1. Configuração global (`~/.config/klautcode/klautcode.json`)
2. Configuração do projeto (`klautcode.json`)
3. Diretório de plugins global (`~/.config/klautcode/plugins/`)
4. Diretório de plugins do projeto (`.klautcode/plugins/`)
...
O klautcode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do klautcode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o klautcode.
...
:::note
Se você estiver usando o aplicativo desktop klautcode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o klautcode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao klautcode:
...
O helper `tool` cria uma ferramenta personalizada que o klautcode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o klautcode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://code.klaut.pro/docs/sdk) para detalhes.
