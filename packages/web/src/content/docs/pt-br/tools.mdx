...
As ferramentas permitem que o LLM execute ações em sua base de código. O klautcode vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no klautcode.
...
:::note
Esta ferramenta está disponível apenas quando `KLAUTCODE_EXPERIMENTAL_LSP_TOOL=true` (ou `KLAUTCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor klautcode ou quando a variável de ambiente `KLAUTCODE_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o klautcode:
