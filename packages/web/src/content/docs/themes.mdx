...
With Klautcode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Klautcode uses our own `klautcode` theme.
...
Klautcode comes with several built-in themes.
...
- Want Klautcode to match their terminal's appearance
...
Klautcode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/klautcode/themes/*.json` or `$XDG_CONFIG_HOME/klautcode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.klautcode/themes/*.json`
4. **Current working directory** - Defined in `./.klautcode/themes/*.json`
