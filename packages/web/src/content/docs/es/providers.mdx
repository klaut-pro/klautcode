---
title: Proveedores
description: Usando cualquier proveedor LLM en Klautcode.
---

import config from "../../../../config.mjs"
export const console = config.console

Klautcode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Klautcode.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Klautcode
configuración.
...
Klautcode Zen es una lista de modelos proporcionados por el equipo Klautcode que han sido
probado y verificado para funcionar bien con Klautcode. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Klautcode Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione klautcode y diríjase a [code.klaut.pro/auth](https://code.klaut.pro/auth).
...
Funciona como cualquier otro proveedor en Klautcode y su uso es completamente opcional.

---

## Klautcode Go

Klautcode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Klautcode que han sido
probado y verificado para funcionar bien con Klautcode.

1. Ejecute el comando `/connect` en TUI, seleccione `Klautcode Go` y diríjase a [code.klaut.pro/auth](https://code.klaut.pro/zen).
...
Para usar Amazon Bedrock con Klautcode:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta klautcode:
   ...
   Para una configuración persistente o específica del proyecto, utilice `klautcode.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Klautcode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar klautcode para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que klautcode funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración klautcode.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `Klautcode`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación klautcode.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Klautcode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Klautcode
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `klautcode.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `klautcode.json`:
...
Para utilizar su suscripción GitHub Copilot con klautcode:
...
Para utilizar Google Vertex AI con Klautcode:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta klautcode.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de klautcode, siempre podrá configurarlo usted mismo.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Klautcode enumerada como una sesión separada.
...
Puede configurar klautcode para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar klautcode para usar modelos locales a través de LM Studio.
...
Puedes configurar klautcode para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Klautcode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/klautcode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Klautcode:
...
3. Copie la clave API para usarla en Klautcode.
...
6. **Importante**: Antes de usar modelos en la nube en Klautcode, debe obtener la información del modelo localmente:
...
Klautcode Zen es una lista de modelos probados y verificados proporcionada por el equipo Klautcode. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Klautcode Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Klautcode Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Klautcode API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración klautcode.
...
5. También puedes personalizarlos a través de tu configuración klautcode. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Klautcode:
...
También puede personalizar modelos a través de su configuración klautcode. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con klautcode. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `klautcode.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Klautcode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `klautcode auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración klautcode y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración klautcode.
