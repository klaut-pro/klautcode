---
title: Servidor
description: Interactuar con el servidor klautcode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `klautcode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente klautcode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `klautcode.local` |
...
Configure `KLAUTCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `klautcode`, o configure `KLAUTCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `klautcode serve` como a `klautcode web`.
...
Cuando ejecuta `klautcode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor klautcode para interactuar con klautcode mediante programación.
:::

Esta arquitectura permite que klautcode admita múltiples clientes y le permite interactuar con klautcode mediante programación.

Puede ejecutar `klautcode serve` para iniciar un servidor independiente. Si tienes el
klautcode TUI ejecutándose, `klautcode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Klautcode [IDE](/docs/ide).
...
El servidor klautcode expone las siguientes API.
