...
Puede agregar herramientas externas a Klautcode utilizando el _Model Context Protocol_ o MCP. Klautcode admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Klautcode Config](https://code.klaut.pro/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/klautcode`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Klautcode maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Klautcode:
...
Si el servidor requiere autenticación, Klautcode le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `klautcode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Klautcode almacenará los tokens de forma segura en `~/.local/share/klautcode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Klautcode, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Klautcode como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Klautcode a su cuenta Sentry.
