...
Las herramientas permiten que LLM realice acciones en su código base. Klautcode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Klautcode.
...
:::note
Esta herramienta solo está disponible cuando `KLAUTCODE_EXPERIMENTAL_LSP_TOOL=true` (o `KLAUTCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Klautcode o cuando la variable de entorno `KLAUTCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Klautcode:

```bash
KLAUTCODE_ENABLE_EXA=1 klautcode
```
