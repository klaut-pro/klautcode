...
Las habilidades del agente permiten a Klautcode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Klautcode busca estas ubicaciones:

- Configuración del proyecto: `.klautcode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/klautcode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Klautcode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.klautcode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/klautcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.klautcode/skills/git-release/SKILL.md` así:
...
Klautcode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `klautcode.json`:
...
**Para agentes integrados** (en `klautcode.json`):
