...
description: Cree herramientas que LLM pueda llamar en klautcode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de klautcode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.klautcode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/klautcode/tools/`.
...
```ts title=".klautcode/tools/database.ts" {1}
...
```ts title=".klautcode/tools/math.ts"
import { tool } from "@klautcode/plugin"
...
```ts title=".klautcode/tools/bash.ts"
import { tool } from "@klautcode/plugin"
...
```ts title=".klautcode/tools/project.ts" {8}
import { tool } from "@klautcode/plugin"
...
```python title=".klautcode/tools/add.py"
import sys
...
```ts title=".klautcode/tools/python-add.ts" {10}
import { tool } from "@klautcode/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".klautcode/tools/add.py")
