---
title: Complementos
description: Escriba sus propios complementos para extender Klautcode.
---

Los complementos le permiten extender Klautcode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Klautcode.
...
- `.klautcode/plugins/` - Complementos a nivel de proyecto
- `~/.config/klautcode/plugins/` - Complementos globales
...
```json title="klautcode.json"
{
  ...
  "plugin": ["klautcode-helicone-session", "klautcode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/klautcode/node_modules/`.
...
1. Configuración global (`~/.config/klautcode/klautcode.json`)
2. Configuración del proyecto (`klautcode.json`)
3. Directorio global de complementos (`~/.config/klautcode/plugins/`)
4. Directorio de complementos del proyecto (`.klautcode/plugins/`)
...
```json title=".klautcode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Klautcode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".klautcode/plugins/my-plugin.ts"
...
```js title=".klautcode/plugins/example.js"
...
- `client`: Un cliente SDK klautcode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar klautcode.
...
```js title=".klautcode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "klautcode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Klautcode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que klautcode lea archivos `.env`:

```javascript title=".klautcode/plugins/env-protection.js"
...
```javascript title=".klautcode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a klautcode:

```ts title=".klautcode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@klautcode/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que klautcode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para klautcode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://code.klaut.pro/docs/sdk) para obtener más detalles.
...
```ts title=".klautcode/plugins/compaction.ts"
import type { Plugin } from "@klautcode/plugin"
...
```ts title=".klautcode/plugins/custom-compaction.ts"
import type { Plugin } from "@klautcode/plugin"
