...
Con Klautcode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Klautcode usa nuestro propio tema `klautcode`.
...
Klautcode viene con varios temas integrados.
...
- Quiere que Klautcode coincida con la apariencia de su terminal
...
Klautcode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/klautcode/themes/*.json` o `$XDG_CONFIG_HOME/klautcode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.klautcode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.klautcode/themes/*.json`
