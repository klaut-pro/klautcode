...
Le skill degli agenti permettono a Klautcode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Klautcode cerca in queste posizioni:

- Config di progetto: `.klautcode/skills/<name>/SKILL.md`
- Config globale: `~/.config/klautcode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Klautcode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.klautcode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/klautcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.klautcode/skills/git-release/SKILL.md` cosi':
...
Klautcode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `klautcode.json`:
...
**Per agenti integrati** (in `klautcode.json`):
