---
title: Server
description: Interagisci con il server klautcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `klautcode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client klautcode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `klautcode.local` |
...
Imposta `KLAUTCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `klautcode`, oppure imposta `KLAUTCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `klautcode serve` sia per `klautcode web`.
...
Quando esegui `klautcode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server klautcode per interagire con klautcode in modo programmatico.
:::

Questa architettura permette a klautcode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `klautcode serve` per avviare un server standalone. Se la TUI di klautcode e gia in esecuzione, `klautcode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Klautcode.
...
Il server klautcode espone le seguenti API.
