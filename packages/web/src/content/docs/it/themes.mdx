...
Con Klautcode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Klautcode usa il tema `klautcode`.
...
Klautcode include diversi temi integrati.
...
- Vuole che Klautcode corrisponda all'aspetto del terminale
...
Klautcode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/klautcode/themes/*.json` o `$XDG_CONFIG_HOME/klautcode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.klautcode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.klautcode/themes/*.json`
