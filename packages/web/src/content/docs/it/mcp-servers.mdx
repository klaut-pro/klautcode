...
Puoi aggiungere strumenti esterni a Klautcode usando il _Model Context Protocol_, o MCP. Klautcode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Klautcode Config](https://code.klaut.pro/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/klautcode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Klautcode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Klautcode:
...
Se il server richiede autenticazione, Klautcode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `klautcode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Klautcode memorizzera' i token in modo sicuro in `~/.local/share/klautcode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Klautcode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Klautcode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Klautcode al tuo account Sentry.
