---
title: Plugin
description: Scrivi plugin per estendere Klautcode.
---

I plugin ti permettono di estendere Klautcode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Klautcode.
...
- `.klautcode/plugins/` - plugin a livello progetto
- `~/.config/klautcode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/klautcode/node_modules/`.
...
1. Config globale (`~/.config/klautcode/klautcode.json`)
2. Config di progetto (`klautcode.json`)
3. Directory plugin globale (`~/.config/klautcode/plugins/`)
4. Directory plugin di progetto (`.klautcode/plugins/`)
...
Klautcode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di klautcode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere klautcode.
...
:::note
Se usi l'app desktop di Klautcode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a klautcode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a klautcode:
...
L'helper `tool` crea uno strumento personalizzato che klautcode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in klautcode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://code.klaut.pro/docs/sdk) per i dettagli.
