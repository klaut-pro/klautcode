...
_Model Bağlam Protokolü_ veya MCP kullanarak klautcode'a harici araçlar ekleyebilirsiniz. klautcode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [klautcode Config](https://code.klaut.pro/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/klautcode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
klautcode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde klautcode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa klautcode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `klautcode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından klautcode, belirteçleri `~/.local/share/klautcode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra klautcode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi klautcode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve klautcode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
