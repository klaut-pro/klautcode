...
klautcode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak klautcode kendi `klautcode` temasını kullanır.
...
klautcode birden fazla yerleşik temayla gelir.
...
- klautcode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
klautcode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/klautcode/themes/*.json` veya `$XDG_CONFIG_HOME/klautcode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.klautcode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.klautcode/themes/*.json`
