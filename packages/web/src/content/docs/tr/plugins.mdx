---
title: Eklentiler
description: Klautcode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Klautcode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Klautcode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.klautcode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/klautcode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/klautcode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/klautcode/klautcode.json`)
2. Project config (`klautcode.json`)
3. Global eklenti dizini (`~/.config/klautcode/plugins/`)
4. Proje eklenti dizini (`.klautcode/plugins/`)
...
Klautcode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Klautcode SDK istemcisi.
...
Klautcode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Klautcode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Klautcode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Klautcode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Klautcode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Klautcode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://code.klaut.pro/docs/sdk) bakın.
