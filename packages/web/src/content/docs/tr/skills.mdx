...
Ajan becerileri, klautcode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
klautcode su konumlari tarar:

- Proje konfigurasyonu: `.klautcode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/klautcode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda klautcode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.klautcode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/klautcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.klautcode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
klautcode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`klautcode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`klautcode.json` icinde):
