...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. klautcode yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
klautcode içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `KLAUTCODE_EXPERIMENTAL_LSP_TOOL=true` (veya `KLAUTCODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca klautcode provider kullanılırken veya `KLAUTCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

klautcode başlatırken etkinleştirmek için:
