...
klautcode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
klautcode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Klautcode, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Klautcode iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `klautcode.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse klautcode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global klautcode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
klautcode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Klautcode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `klautcode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `klautcode models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/klaut-pro/klautcode).
:::
