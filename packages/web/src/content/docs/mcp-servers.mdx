...
You can add external tools to Klautcode using the _Model Context Protocol_, or MCP. Klautcode supports both local and remote servers.
...
You can define MCP servers in your [Klautcode Config](https://code.klaut.pro/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/klautcode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Klautcode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Klautcode will:
...
If the server requires authentication, Klautcode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `klautcode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Klautcode will store the tokens securely in `~/.local/share/klautcode/mcp-auth.json`.
...
Your MCPs are available as tools in Klautcode, alongside built-in tools. So you can manage them through the Klautcode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Klautcode to your Sentry account.
