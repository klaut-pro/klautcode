...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Klautcode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Klautcode.
...
:::note
Ovaj alat je dostupan samo kada je `KLAUTCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `KLAUTCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Klautcode provajdera ili kada je varijabla `KLAUTCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Klautcode:
