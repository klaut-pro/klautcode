...
Da biste otklonili probleme s Klautcode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\klautcode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `klautcode --log-level DEBUG`.

---

## Pohrana

Klautcode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/klautcode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\klautcode`
...
Klautcode Desktop pokreće lokalni Klautcode server (`klautcode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Klautcode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/klautcode/klautcode.jsonc` (ili `~/.config/klautcode/klautcode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/klautcode/klautcode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\klautcode\klautcode.jsonc`
...
Klautcode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/klautcode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\klautcode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Klautcode mogao ponovo izgraditi.

1. Potpuno zatvorite Klautcode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/klautcode`
- **Linux**: obrišite `~/.cache/klautcode` (ili pokrenite `rm -rf ~/.cache/klautcode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\klautcode`

3. Ponovo pokrenite Klautcode Desktop.

---

### Rješavanje problema sa vezom na serveru

Klautcode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `klautcode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `KLAUTCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `KLAUTCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Klautcode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Klautcode karakteristikama.

---

### Obavještenja se ne prikazuju

Klautcode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Klautcode u postavkama vašeg OS-a, i
...
1. Zatvorite Klautcode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Klautcode Desktop aplikacije):

- `klautcode.settings.dat` (URL zadanog servera za desktop)
- `klautcode.global.dat` i `klautcode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s Klautcode:
...
Da saznate kojim modelima imate pristup, pokrenite `klautcode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\klautcode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Klautcode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\klautcode`

2. Ponovo pokrenite Klautcode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Klautcode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Klautcode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
