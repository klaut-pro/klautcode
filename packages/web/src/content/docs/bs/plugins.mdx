---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Klautcode.
---

Dodaci vam omogućavaju da proširite Klautcode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Klautcode.
...
- `.klautcode/plugins/` - Dodaci na nivou projekta
- `~/.config/klautcode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/klautcode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/klautcode/klautcode.json`)
2. Konfiguracija projekta (`klautcode.json`)
3. Globalni direktorij dodataka (`~/.config/klautcode/plugins/`)
4. Direktorij dodataka projekta (`.klautcode/plugins/`)
...
Klautcode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Klautcode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Klautcode.
...
Spriječite klautcode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u klautcode:
...
Pomoćnik `tool` kreira prilagođeni alat koji klautcode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za klautcode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://code.klaut.pro/docs/sdk) za detalje.
