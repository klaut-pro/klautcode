...
Možete dodati vanjske alate u Klautcode koristeći _Model Context Protocol_, ili MCP. Klautcode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Klautcode Config](https://code.klaut.pro/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/klautcode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Klautcode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Klautcode će:
...
Ako server zahtijeva autentifikaciju, Klautcode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `klautcode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Klautcode sigurno čuva tokene u `~/.local/share/klautcode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Klautcode, zajedno s ugrađenim alatima. Možete njima upravljati kroz Klautcode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Klautcode sa vašim Sentry nalogom.
