---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Klautcode.
---

import config from "../../../../config.mjs"
export const console = config.console

Klautcode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Klautcode konfiguraciji.
...
u `~/.local/share/klautcode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Klautcode
config.
...
Klautcode Zen je lista modela koje je obezbedio Klautcode tim koji su bili
testirano i potvrđeno da dobro radi sa Klautcode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Klautcode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite klautcode i idite na [code.klaut.pro/auth](https://code.klaut.pro/auth).
...
Radi kao i svaki drugi provajder u Klautcode i potpuno je opcionalan za korištenje.

---

## Klautcode Go

Klautcode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Klautcode tim i koji su testirani i verificirani da dobro rade s Klautcode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Klautcode Go` i idite na [code.klaut.pro/auth](https://code.klaut.pro/zen).
...
Da biste koristili Amazon Bedrock s Klautcode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete klautcode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `klautcode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Klautcode.
:::
...
Možete konfigurirati klautcode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi klautcode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju klautcode konfiguraciju.
...
da pohrani token u klautcode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Klautcode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Klautcode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`klautcode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `klautcode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s klautcode:
...
Za korištenje Google Vertex AI s Klautcode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće klautcode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem klautcodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [klautcode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki Klautcode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Klautcode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati klautcode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati klautcode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati klautcode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Klautcode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/klautcode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Klautcode:
...
3. Kopirajte API ključ za korištenje u Klautcode.
...
6. **Važno**: Prije upotrebe modela oblaka u Klautcode, morate lokalno povući informacije o modelu:
...
Klautcode Zen je lista testiranih i verifikovanih modela koju je obezbedio Klautcode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Klautcode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Klautcode Zen**.

```txt
   /connect
```

3. Unesite svoj Klautcode API ključ.
...
Također možete dodati dodatne modele putem vaše klautcode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše klautcode konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Klautcodeom:
...
Također možete prilagoditi modele kroz svoju klautcode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s klautcode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `klautcode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Klautcode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `klautcode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Klautcode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj klautcode konfiguraciji.
