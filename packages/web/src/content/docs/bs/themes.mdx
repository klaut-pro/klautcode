...
U Klautcode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Klautcode koristi nasu `klautcode` temu.
...
Klautcode dolazi sa vise ugradenih tema.
...
- Zele da Klautcode odgovara izgledu njihovog terminala
...
Klautcode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/klautcode/themes/*.json` ili `$XDG_CONFIG_HOME/klautcode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.klautcode/themes/*.json`
4. **Trenutni radni direktorij** - `./.klautcode/themes/*.json`
