---
title: Server
description: Komunicirajte s klautcode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `klautcode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji klautcode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `klautcode.local` |
...
Postavite `KLAUTCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `klautcode`, ili postavite `KLAUTCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `klautcode serve` i za `klautcode web`.
...
Kada pokrenete `klautcode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite klautcode server za programsku interakciju sa klautcode-om.
:::

Ova arhitektura omogucava klautcode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `klautcode serve` da startate standalone server. Ako je klautcode TUI vec pokrenut, `klautcode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Klautcode [IDE](/docs/ide) pluginovi.
...
klautcode server izlaže sljedece API-je.
