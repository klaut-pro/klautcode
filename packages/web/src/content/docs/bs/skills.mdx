...
Agent skills omogucavaju Klautcode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Klautcode pretrazuje ove lokacije:

- Konfiguracija projekta: `.klautcode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/klautcode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Klautcode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.klautcode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/klautcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.klautcode/skills/git-release/SKILL.md` ovako:
...
Klautcode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `klautcode.json`:
...
**Za ugradene agente** (u `klautcode.json`):
