---
title: Konfiguracija
description: Korištenje Klautcode JSON konfiguracije.
---

Možete konfigurirati Klautcode koristeći JSON konfiguracijski fajl.

---

## Format

Klautcode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/klautcode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/klautcode/klautcode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`KLAUTCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`klautcode.json` u projektu) - postavke specifične za projekat
5. **`.klautcode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`KLAUTCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.klautcode` i `~/.config/klautcode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/klautcode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Klautcode konfiguraciju u `~/.config/klautcode/klautcode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/klautcode/tui.json`.
...
Dodajte `klautcode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Klautcode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `KLAUTCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `KLAUTCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.klautcode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.klautcode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`code.klaut.pro/config.json`**](https://code.klaut.pro/config.json).

TUI konfiguracija koristi [**`code.klaut.pro/tui.json`**](https://code.klaut.pro/tui.json).
...
Koristite `KLAUTCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `klautcode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `klautcode serve` i `klautcode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Klautcode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `klautcode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Klautcode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Klautcode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/klautcode/agents/` ili `.klautcode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Klautcode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`klautcode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/klautcode/commands/` ili `.klautcode/commands/`. [Saznajte više ovdje](/docs/commands).
...
Klautcode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Klautcode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Klautcode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.klautcode/plugins/` ili `~/.config/klautcode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Klautcode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
