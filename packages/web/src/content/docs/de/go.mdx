...
Klautcode Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Klautcode. Du abonnierst Klautcode Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
Klautcode zu verwenden.
...
Klautcode Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Klautcode Go funktioniert wie jeder andere Provider in Klautcode.

1. Du meldest dich bei **<a href={console}>Klautcode Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Klautcode Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Klautcode Go abonnieren.
:::
...
Klautcode Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Klautcode Config verwendet das Format `klautcode-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `klautcode-go/kimi-k3` in deiner Config verwenden.
...
Wir haben Klautcode Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Klautcode zu nutzen.
