---
title: Server
description: Nutze den klautcode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `klautcode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein klautcode-Client nutzen kann.
...
Setze `KLAUTCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `klautcode`, kann aber mit `KLAUTCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `klautcode serve` und `klautcode web`.
...
Wenn du `klautcode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den klautcode-Server, um klautcode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `klautcode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `klautcode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Klautcode-[IDE](/docs/ide)-Plugins.
...
Der klautcode-Server stellt folgende APIs bereit.
