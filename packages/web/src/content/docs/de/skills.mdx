...
Agent Skills erlauben Klautcode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Klautcode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Klautcode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.klautcode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/klautcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.klautcode/skills/git-release/SKILL.md` zum Beispiel so:
...
Klautcode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `klautcode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `klautcode.json`):
