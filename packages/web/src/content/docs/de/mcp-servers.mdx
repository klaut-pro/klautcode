...
Sie können externe Tools zu Klautcode hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Klautcode unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Klautcode Config](https://code.klaut.pro/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/klautcode`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Klautcode übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Klautcode Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Klautcode beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `klautcode mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Klautcode die Token sicher in `~/.local/share/klautcode/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Klautcode verfügbar. Sie können sie also wie jedes andere Tool über die Klautcode-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Klautcode mit Ihrem Sentry-Konto zu verbinden.
