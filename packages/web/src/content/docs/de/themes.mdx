...
Mit Klautcode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Klautcode das `klautcode`-Theme.
...
Klautcode liefert mehrere eingebaute Themes mit.
...
- Klautcode optisch ans Terminal anpassen wollen
...
Klautcode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/klautcode/themes/*.json` oder `$XDG_CONFIG_HOME/klautcode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.klautcode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.klautcode/themes/*.json`
