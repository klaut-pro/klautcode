---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Klautcode zu erweitern.
---

Mit Plugins können Sie Klautcode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Klautcode zu ändern.
...
- `.klautcode/plugins/` – Plugins auf Projektebene
- `~/.config/klautcode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/klautcode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/klautcode/klautcode.json`)
2. Projektkonfiguration (`klautcode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/klautcode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.klautcode/plugins/`)
...
Klautcode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Klautcode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Klautcode erweitern können.
...
:::note
Wenn Sie die Klautcode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Klautcode `.env`-Dateien liest:
...
Plugins können Klautcode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Klautcode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Klautcode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://code.klaut.pro/docs/sdk).
