---
title: Regeln
description: Lege eigene Anweisungen fuer klautcode fest.
---

Du kannst klautcode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in klautcode den Befehl `/init` aus.
...
So kann klautcode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht klautcode sie zu erweitern.
...
klautcode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/klautcode/AGENTS.md` speichern.
Diese gelten in allen klautcode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Klautcode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/klautcode/AGENTS.md` existiert)
...
Beim Start sucht klautcode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/klautcode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `klautcode.json` oder in `~/.config/klautcode/klautcode.json` angeben.
...
klautcode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit klautcode.json

Empfohlen ist das `instructions`-Feld in der `klautcode.json`:
...
Du kannst klautcode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `klautcode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
