...
There are two types of agents in Klautcode; primary agents and subagents.
...
Klautcode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Klautcode comes with two built-in subagents, **Explore** and **Scout**, plus **General** which doubles as a primary agent. We'll look at these below.

---

## Built-in

Klautcode comes with two built-in primary agents, a multi-mode agent, and two built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into Klautcode's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `klautcode.json` config file:
...
- Global: `~/.config/klautcode/agents/`
- Per-project: `.klautcode/agents/`

```markdown title="~/.config/klautcode/agents/review.md"
---
...
If no temperature is specified, Klautcode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Klautcode config and the project specific config.
...
The model ID in your Klautcode config uses the format `provider/model-id`. For example, if you're using [Klautcode Zen](/docs/zen), you would use `klautcode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="klautcode.json" {3-6,9-12}
{
  "$schema": "https://code.klaut.pro/config.json",
  "tools": {
    "write": true,
...
```json title="klautcode.json" {3-5,8-10}
{
  "$schema": "https://code.klaut.pro/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `klautcode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/klaut-pro/klautcode).
:::
...
```markdown title="~/.config/klautcode/agents/docs-writer.md"
---
...
```markdown title="~/.config/klautcode/agents/security-auditor.md"
---
