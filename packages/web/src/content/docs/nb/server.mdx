---
title: Tjener
description: Samhandle med Klautcode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `klautcode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Klautcode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `klautcode.local` |
...
Sett `KLAUTCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `klautcode` som standard, eller sett `KLAUTCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `klautcode serve` og `klautcode web`.
...
Når du kjører `klautcode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Klautcode-serveren til å samhandle med Klautcode programmatisk.
:::

Denne arkitekturen lar Klautcode støtte flere klienter og lar deg samhandle med Klautcode programmatisk.

Du kan kjøre `klautcode serve` for å starte en frittstående server. Hvis du har
Klautcode TUI kjører, vil `klautcode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Klautcode [IDE](/docs/ide) plugins.
...
klautcode-serveren viser følgende APIer.
