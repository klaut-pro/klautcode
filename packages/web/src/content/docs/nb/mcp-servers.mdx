...
Du kan legge til eksterne verktøy i Klautcode ved å bruke _Model Context Protocol_ eller MCP. Klautcode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Klautcode Config](https://code.klaut.pro/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="klautcode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/klautcode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="klautcode.jsonc" {15}
{
  "$schema": "https://code.klaut.pro/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
Klautcode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Klautcode:
...
Hvis serveren krever autentisering, vil Klautcode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `klautcode mcp auth <server-name>`.
...
```json title="klautcode.json" {7-11}
{
  "$schema": "https://code.klaut.pro/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
klautcode mcp auth my-oauth-server
```
...
```bash
klautcode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Klautcode lagre tokens sikkert i `~/.local/share/klautcode/mcp-auth.json`.
...
klautcode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Klautcode, sammen med innebygde verktøy. Så du kan administrere dem gjennom Klautcode-konfigurasjonen som alle andre verktøy.
...
```json title="klautcode.json" {11, 14-18}
{
  "$schema": "https://code.klaut.pro/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
klautcode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Klautcode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Klautcode-samtale oppført som en separat økt.
