---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Klautcode.
---

import config from "../../../../config.mjs"
export const console = config.console

Klautcode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Klautcode-konfigurasjonen.
...
i `~/.local/share/klautcode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Klautcode
konfig.
...
Klautcode Zen er en liste over modeller levert av Klautcode-teamet som har vært
testet og verifisert for å fungere godt med Klautcode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Klautcode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Klautcode Zen` og gå til [code.klaut.pro/auth](https://code.klaut.pro/zen).
...
Det fungerer som alle andre leverandører i Klautcode og er helt valgfritt å bruke.

---

## Klautcode Go

Klautcode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Klautcode-teamet som har vært
testet og verifisert for å fungere godt med Klautcode.

1. Kjør kommandoen `/connect` i TUI, velg `Klautcode Go`, og gå til [code.klaut.pro/auth](https://code.klaut.pro/zen).
...
Slik bruker du Amazon Bedrock med Klautcode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Klautcode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `klautcode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i klautcode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere klautcode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Klautcode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Klautcode-konfigurasjonen.
...
for å lagre token i Klautcode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Klautcode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Klautcode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`klautcode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Tilpass gjennom `klautcode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Klautcode:
...
Slik bruker du Google Vertex AI med Klautcode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Klautcode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Klautcode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [klautcode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver Klautcode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Klautcode-samtale oppført som en separat økt.
...
Du kan konfigurere Klautcode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Klautcode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Klautcode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Klautcode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/klautcode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Klautcode:
...
3. Kopier API-nøkkelen for bruk i Klautcode.
...
6. **Viktig**: Før du bruker skymodeller i Klautcode, må du hente modellinformasjonen lokalt:
...
Klautcode Zen er en liste over testede og verifiserte modeller levert av Klautcode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Klautcode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Klautcode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Klautcode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Klautcode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Klautcode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Klautcode:
...
Du kan også tilpasse modeller gjennom Klautcode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Klautcode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `klautcode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Klautcode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `klautcode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Klautcode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Klautcode-konfigurasjonen.
