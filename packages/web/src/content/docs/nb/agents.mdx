...
Det er to typer agenter i Klautcode; primære agenter og underagenter.
...
Klautcode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Klautcode kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Klautcode kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `klautcode.json`:
...
Hvis ingen temperatur er spesifisert, bruker Klautcode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Klautcode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Klautcode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Klautcode Zen](/docs/zen), vil du bruke `klautcode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `klautcode models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/klaut-pro/klautcode).
:::
