...
description: Skriv dine egne programtillegg for å utvide Klautcode.
---

Plugins lar deg utvide Klautcode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Klautcode.
...
- `.klautcode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/klautcode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/klautcode/klautcode.json`)
2. Prosjektkonfigurasjon (`klautcode.json`)
3. Global plugin-katalog (`~/.config/klautcode/plugins/`)
4. Prosjektpluginkatalog (`.klautcode/plugins/`)
...
Klautcode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Klautcode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Klautcode.
...
:::note
Hvis du bruker Klautcode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Klautcode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Klautcode:
...
`tool`-hjelperen lager et tilpasset verktøy som Klautcode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Klautcode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://code.klaut.pro/docs/sdk) for detaljer.
