...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في klautcode.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في klautcode مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.klautcode/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/klautcode/tools/`.
