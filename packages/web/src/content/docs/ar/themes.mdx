...
مع Klautcode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Klautcode سمتنا `klautcode`.
...
يأتي Klautcode مع عدة سمات مدمجة.
...
- يريدون أن يطابق Klautcode مظهر terminal لديهم
...
يدعم Klautcode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/klautcode/themes/*.json` أو `$XDG_CONFIG_HOME/klautcode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.klautcode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.klautcode/themes/*.json`
