---
title: الخادم
description: تفاعل مع خادم klautcode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `klautcode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل klautcode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `klautcode.local` |
...
عيّن `KLAUTCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `klautcode`، أو عيّن `KLAUTCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `klautcode serve` و `klautcode web`.
...
عند تشغيل `klautcode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم klautcode للتفاعل مع klautcode برمجيا.
:::

تتيح هذه البنية لـ klautcode دعم عدة عملاء وتمكّنك من التفاعل مع klautcode برمجيا.

يمكنك تشغيل `klautcode serve` لبدء خادم مستقل. إذا كانت واجهة klautcode في terminal (TUI)
قيد التشغيل، فسيبدأ `klautcode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Klautcode لـ [IDE](/docs/ide).
...
يعرض خادم klautcode واجهات API التالية.
