---
title: المزوّدون
description: استخدام أي موفّر LLM في Klautcode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Klautcode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Klautcode.
...
في `~/.local/share/klautcode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Klautcode.
...
Klautcode Zen هي قائمة نماذج يوفّرها فريق Klautcode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Klautcode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Klautcode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر klautcode، ثم انتقل إلى [code.klaut.pro/auth](https://code.klaut.pro/auth).
...
يعمل مثل أي مزوّد آخر في Klautcode واستخدامه اختياري بالكامل.

---

## Klautcode Go

Klautcode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Klautcode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Klautcode.

1. شغّل الأمر `/connect` في TUI، واختر `Klautcode Go`، ثم انتقل إلى [code.klaut.pro/auth](https://code.klaut.pro/zen).
...
لاستخدام Amazon Bedrock مع Klautcode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل klautcode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `klautcode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Klautcode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين klautcode لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل klautcode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات klautcode.
...
تخزين الرمز في مساحة تخزين مصادقة klautcode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Klautcode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Klautcode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`klautcode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع klautcode:
...
لاستخدام Google Vertex AI مع Klautcode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر klautcode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [klautcode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة Klautcode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Klautcode مدرجة كجلسة مستقلة.
...
يمكنك ضبط klautcode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط klautcode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط klautcode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Klautcode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/klautcode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Klautcode:
...
3. انسخ مفتاح API لاستخدامه في Klautcode.
...
6. **مهم**: قبل استخدام نماذج السحابة في Klautcode، يجب سحب معلومات النموذج محليا:
...
Klautcode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Klautcode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Klautcode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Klautcode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Klautcode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات klautcode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات klautcode. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Klautcode:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات klautcode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع klautcode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `klautcode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Klautcode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `klautcode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات klautcode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات klautcode.
