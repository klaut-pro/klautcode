...
Создайте `.klautcode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию klautcode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем klautcode [config](/docs/config):
...
- Глобальный: `~/.config/klautcode/commands/`
- Для каждого проекта: `.klautcode/commands/`
...
klautcode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
