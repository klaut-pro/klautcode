...
import type { SessionNotFoundError } from "@klautcode/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em klautcode.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
