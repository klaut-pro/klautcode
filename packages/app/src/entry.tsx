...
const DEFAULT_SERVER_URL_KEY = "klautcode.settings.dat:defaultServerUrl"
...
const getCurrentUrl = () => {
  if (location.hostname.includes("code.klaut.pro")) return "http://localhost:4096"
  if (import.meta.env.DEV)
    return `http://${import.meta.env.VITE_KLAUTCODE_SERVER_HOST ?? "localhost"}:${import.meta.env.VITE_KLAUTCODE_SERVER_PORT ?? "4096"}`
  ...
  Sentry.init({
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" && !(import.meta.env.KLAUTCODE_CHANNEL === "prod" && i.name === "GlobalHandlers"),
      )
    },
  })
}
