...
export const loadGlobalConfigQuery = (scope: ServerScope, sdk: KlautcodeClient, protocol?: Promise<ServerProtocol>) =>
  queryOptions({
    ...
    queryFn: () =>
      retry(() =>
        api.list().then((projects) => {
          return projects
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("klautcode-test"))
  ...
  ;(async () => {
    const slow = [
      ...
      input.mcp &&
        (() =>
          input.queryClient.fetchQuery(
            loadMcpQuery(input.scope, input.directory, input.sdk),
          )),
      input.mcp &&
        (() =>
          input.queryClient.fetchQuery(
