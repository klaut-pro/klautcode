...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("klautcode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("klautcode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("klautcode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("klautcode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("klautcode://open-project")).toBeUndefined()
    expect(parseDeepLink("klautcode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "klautcode://open-project?directory=/a",
      "klautcode://other?directory=/b",
      "klautcode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("klautcode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("klautcode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("klautcode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("klautcode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "klautcode://new-session?directory=/a",
      "klautcode://open-project?directory=/b",
      "klautcode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __KLAUTCODE__: {
        deepLinks: ["klautcode://open-project?directory=/a"],
      },
    } as unknown as Window & { __KLAUTCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["klautcode://open-project?directory=/a"])
