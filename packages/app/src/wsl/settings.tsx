...
import { wslKlautcodeAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.klautcodeChecks[item.config.distro]
          const klautcodeAction = () => wslKlautcodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-klautcode" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={klautcodeAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installKlautcode(item.config.distro))}
                    >
