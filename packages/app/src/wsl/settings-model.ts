...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-klautcode" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslKlautcodeAction(check?: WslKlautcodeCheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installKlautcode"
  if (check.matchesDesktop === false) return "wsl.onboarding.updateKlautcode"
}
...
export function addServerViewModel(input: {
  ...
  const klautcodeCheck = selectedDistro ? (state?.klautcodeChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.klautcodeChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateKlautcode" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.klautcodeMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installKlautcode" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  klautcodeCheck: WslKlautcodeCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const probingKlautcode =
    probingSelected ||
    (ready &&
      (!input.klautcodeCheck ||
        (!!input.selectedDistro &&
          input.state?.job?.kind === "probe-addable" &&
          input.state.job.distros.includes(input.selectedDistro))))
  const installingKlautcode =
    input.state?.job?.kind === "install-klautcode" && input.state.job.distro === input.selectedDistro
  if (!ready || probingKlautcode) {
    return {
      variant: "contrast",
  ...
  if (!addServerKlautcodeReady(input.klautcodeCheck)) {
    const update = !!input.klautcodeCheck?.resolvedPath && input.klautcodeCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingKlautcode
        ? { key: "wsl.onboarding.updatingKlautcode" }
        : update
          ? { key: "wsl.onboarding.updateKlautcode" }
          : { key: "wsl.onboarding.installKlautcode" },
      disabled: !!input.state?.job || input.adding,
      action: "install-klautcode",
      loading: installingKlautcode,
...
function addServerKlautcodeReady(check: WslKlautcodeCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.klautcodeChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.klautcodeChecks[item.name]) return [`klautcode:${item.name}`]
    return []
  })
