import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@klautcode/client/promise"
...
import { List, type ListRef } from "@klautcode/ui/list"
import { ProviderIcon } from "@klautcode/ui/provider-icon"
import { Spinner } from "@klautcode/ui/spinner"
import { Tag } from "@klautcode/ui/tag"
import { TextField } from "@klautcode/ui/text-field"
import { ButtonV2 } from "@klautcode/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@klautcode/ui/v2/dialog-v2"
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "klautcode"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.klautcode.tagline")}</div>
          </Show>
  ...
  function ApiAuthView() {
        ...
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          ...
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.klautcodeZen.line1")}</div>
              <div>{language.t("provider.connect.klautcodeZen.line2")}</div>
              <div>
                {language.t("provider.connect.klautcodeZen.visit.prefix")}
                <ExternalLink
                  href="https://code.klaut.pro/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.klautcodeZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.klautcodeZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "klautcode"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.klautcodeZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.klautcodeZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.klautcodeZen.visit.prefix")}
                <ExternalLink href="https://code.klaut.pro/zen" tabIndex={-1}>
                  {language.t("provider.connect.klautcodeZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.klautcodeZen.visit.suffix")}
              </div>
