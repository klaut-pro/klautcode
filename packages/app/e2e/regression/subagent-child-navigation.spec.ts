...
import { currentSession, mockKlautcodeServer } from "../utils/mock-server"
import { expectSessionTitle } from "../utils/waits"

const directory = "C:/Klautcode/SubagentNavigation"
...
test("navigates to a subagent child session missing from the session list", async ({ page }) => {
  ...
  const titlebarRight = page.locator("#klautcode-titlebar-right")
...
async function setup(page: Page, events?: () => EventPayload[]) {
  await mockKlautcodeServer(page, {
    ...
    provider: {
      ...
      connected: ["klautcode"],
      default: { providerID: "klautcode", modelID: "claude-opus-4-6" },
    },
  ...
  return [
    {
      info: {
        ...
        agent: "build",
        model: { providerID: "klautcode", modelID: "claude-opus-4-6" },
      },
      parts: [
        {
...
async function configurePage(page: Page) {
  ...
  await page.addInitScript(
    ({ directory, server, sessionId }) => {
      ...
      localStorage.setItem("klautcode.window.browser.dat:tabs", JSON.stringify([{ type: "session", server, sessionId }]))
    },
