...
import { SessionStatusEvent } from "@klautcode/schema/session-status-event"
import { SessionV1 } from "@klautcode/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/Klautcode/TimelineStability"
...
export const model = { providerID: "klautcode", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("klautcode.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
