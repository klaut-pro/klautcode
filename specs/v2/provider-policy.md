...
Both `action` and `resource` use klautcode's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.klautcode` files is intentionally deferred until `.klautcode` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether klautcode permits an operation through its managed execution paths.
