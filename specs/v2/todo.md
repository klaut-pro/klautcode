...
The klautcode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@klautcode/llm` tool loop after replacing its
...
- plugins get global "klautcode" instance like in that post i showed
- klautcode instance has stuff like `klautcode.session.prompt()` or
  `klautcode.tool.register({...})`
...
- decide whether to preserve deprecated `@klautcode/llm` orchestration exports
