...
Move the canonical Klautcode terminal application from
`packages/klautcode/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/klautcode`,
`packages/cli`, or `@klautcode/core`.

The SDK is the TUI's Klautcode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/klautcode`.
- Do not replace private `packages/klautcode` imports with `@klautcode/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@klautcode/tui`.
...
- Klautcode domain data displayed by the TUI
...
```text
packages/klautcode/src/cli/cmd/tui
```

Its private dependency on `packages/klautcode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/klautcode/src/*`.
...
Status: Completed. The private `@klautcode/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@klautcode/tui`.
...
- The package has no dependency on `klautcode`, `@klautcode/cli`, or
  `@klautcode/core`.
...
primitives now live in `@klautcode/tui`. The legacy host consumes explicit
...
and build inputs through `@klautcode/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Klautcode globals or process state;
...
Replace process-global Klautcode state with resolved TUI inputs.
...
`@klautcode/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/klautcode` initially.
...
canonical in `@klautcode/tui`. Configured references resolve through the new
...
adapters rather than Klautcode domain implementations.

Make the SDK the only Klautcode domain boundary used by the TUI.
...
- Domain-facing TUI code imports Klautcode data and operations only from
  `@klautcode/sdk`.
...
`@klautcode/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/klautcode`.
...
- The package has no imports from `packages/klautcode`, `packages/cli`, or
  `@klautcode/core`.
...
public `@klautcode/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/klautcode`.
- Keep the legacy embedded worker and server startup in `packages/klautcode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@klautcode/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/klautcode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@klautcode/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@klautcode/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Klautcode domain state.
...
cd packages/klautcode && bun typecheck
...
rg '@klautcode/core|packages/klautcode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/klautcode/src packages/klautcode/test
```
...
- Build the current-platform `packages/klautcode` binary.
