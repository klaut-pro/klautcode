# Remove `packages/klautcode/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/klautcode/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the klautcode legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/klautcode/src/storage/db.ts` are concentrated in 22 source files:

- `packages/klautcode/src/account/repo.ts`
- `packages/klautcode/src/cli/cmd/db.ts`
- `packages/klautcode/src/cli/cmd/import.ts`
- `packages/klautcode/src/cli/cmd/stats.ts`
- `packages/klautcode/src/control-plane/workspace.ts`
- `packages/klautcode/src/index.ts`
- `packages/klautcode/src/node.ts`
- `packages/klautcode/src/permission/index.ts`
- `packages/klautcode/src/project/project.ts`
- `packages/klautcode/src/server/projectors.ts`
- `packages/klautcode/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/klautcode/src/server/shared/fence.ts`
- `packages/klautcode/src/session/message-v2.ts`
- `packages/klautcode/src/session/projectors.ts`
- `packages/klautcode/src/session/prompt.ts`
- `packages/klautcode/src/session/session.ts`
- `packages/klautcode/src/session/todo.ts`
- `packages/klautcode/src/share/share-next.ts`
- `packages/klautcode/src/storage/db.ts`
- `packages/klautcode/src/sync/index.ts`
- `packages/klautcode/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy klautcode database wrapper; `packages/klautcode/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the klautcode wrapper.
...
Status: Completed. `SyncEvent` and the klautcode projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy klautcode database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy klautcode database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy klautcode database wrapper.
...
All migration groups are complete or superseded. `packages/klautcode/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No klautcode data-migration group remains.

The previous klautcode `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate klautcode data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/klautcode`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/klautcode/src`
- `bun typecheck` from `packages/klautcode`
- Relevant package tests from `packages/klautcode`, not the repo root
