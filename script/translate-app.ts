...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "klautcode/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".klautcode/glossary/zh-cn.md"
  if (locale === "zht") return ".klautcode/glossary/zh-tw.md"
  return `.klautcode/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Klautcode did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Klautcode returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Klautcode session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Klautcode runs for 'all' (default: 4)
      --model <provider/id>  Klautcode model (default: klautcode/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Klautcode
...
`)
  ...
  if (failed.length) console.error(`\nKlautcode failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.KLAUTCODE_DISABLE_PROJECT_CONFIG = "1"
  env.KLAUTCODE_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["klautcode", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.KLAUTCODE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["klautcode", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.KLAUTCODE_CONFIG
  delete env.KLAUTCODE_CONFIG_DIR
  delete env.KLAUTCODE_CONFIG_CONTENT
  delete env.KLAUTCODE_PERMISSION
  delete env.KLAUTCODE_AUTO_SHARE
  return env
}
