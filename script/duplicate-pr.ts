...
import { createKlautcode } from "@klautcode/sdk"
...
`)
  ...
  const klautcode = await createKlautcode({ port: 0 })
  ...
  try {
    ...
    const session = await klautcode.client.session.create()
    const result = await klautcode.client.session
      .prompt({
