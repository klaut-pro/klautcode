...
# 패키지 매니저
npm i -g klautcode-ai@latest        # bun/pnpm/yarn 도 가능
scoop install klautcode             # Windows
choco install klautcode             # Windows
brew install klaut-pro/tap/klautcode # macOS 및 Linux (권장, 항상 최신)
brew install klautcode              # macOS 및 Linux (공식 brew formula, 업데이트 빈도 낮음)
sudo pacman -S klautcode            # Arch Linux (Stable)
paru -S klautcode-bin               # Arch Linux (Latest from AUR)
mise use -g klautcode               # 어떤 OS든
nix run nixpkgs#klautcode           # 또는 github:klaut-pro/klautcode 로 최신 dev 브랜치
```
...
Klautcode 는 데스크톱 앱으로도 제공됩니다. [releases page](https://github.com/klaut-pro/klautcode/releases) 에서 직접 다운로드하거나 [code.klaut.pro/download](https://code.klaut.pro/download) 를 이용하세요.
...
Klautcode 에는 내장 에이전트 2개가 있으며 `Tab` 키로 전환할 수 있습니다.
...
[agents](https://code.klaut.pro/docs/agents) 에 대해 더 알아보세요.

### 문서

Klautcode 설정에 대한 자세한 내용은 [**문서**](https://code.klaut.pro/docs) 를 참고하세요.

### 기여하기

Klautcode 에 기여하고 싶다면, Pull Request 를 제출하기 전에 [contributing docs](./CONTRIBUTING.md) 를 읽어주세요.

### Klautcode 기반으로 만들기

Klautcode 와 관련된 프로젝트를 진행하면서 이름에 "klautcode"(예: "klautcode-dashboard" 또는 "klautcode-mobile") 를 포함한다면, README 에 해당 프로젝트가 Klautcode 팀이 만든 것이 아니며 어떤 방식으로도 우리와 제휴되어 있지 않다는 점을 명시해 주세요.

---

**커뮤니티에 참여하기** [Discord](https://discord.gg/klautcode) | [X.com](https://x.com/klautcode)
